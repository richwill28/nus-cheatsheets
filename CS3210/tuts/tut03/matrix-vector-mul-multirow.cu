...
__global__ void multiplyKernel(float *c, const float *a, const float *b) {
    int index = threadIdx.x + blockIdx.x * blockDim.x;
    	...
    	for (int trow = 0; trow < ROWS_PER_THREAD; trow++) {
		int rowidx = index * ROWS_PER_THREAD + trow;
		c[rowidx] = 0;
		for (int j = 0; j < ARRAYSIZE; ++j) {
			c[index] += a[rowidx * ARRAYSIZE + j] * b[rowidx];
		}
	}
}
...
cudaError_t multiplyWithCuda(float *c, const float *a, const float *b)
{
    ...
    multiplyKernel<<<ARRAYSIZE/BLOCKSIZE/ROWS_PER_THREAD, BLOCKSIZE>>>(dev_c, dev_a, dev_b);
